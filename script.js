...
// Initialize boot sequence - Mobile compatible version
setTimeout(() => {
  const promptElement = bootSequence.lastElementChild;
  promptElement.textContent = "TAP ANYWHERE TO CONTINUE";
  
  const continueHandler = () => {
    document.removeEventListener('click', continueHandler);
    document.removeEventListener('touchstart', continueHandler);
    promptElement.removeEventListener('click', continueHandler);
    
    const playSoundWithCheck = () => {
      try {
  ...
  document.addEventListener('touchstart', continueHandler, { passive: true });
  promptElement.addEventListener('click', continueHandler);
}, 6000);
